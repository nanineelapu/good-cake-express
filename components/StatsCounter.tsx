...
function StatItem({ stat }: { stat: any }) {
  ...
  return (
    <div ref={elementRef} className="flex flex-col items-center group">
      <div className="mb-6 p-4 rounded-2xl bg-black/5 border border-black/5 group-hover:bg-primary/5 group-hover:border-primary/10 transition-all duration-300">
        <stat.icon size={28} className="text-primary group-hover:text-primary transition-colors" />
      </div>
      <div className="text-4xl md:text-5xl font-display font-bold text-text mb-2 tracking-tighter">
        {count}{stat.suffix}
      </div>
